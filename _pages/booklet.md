...
It is very common to use functions that have to work on arrays. Obviously, you can easily pass a single element of an array to a function. That's really not problematic at all. But, if you need the function to work on the whole array you have to take another strategy. Unfortunately, you can not pass an array to a function, this is because it would mean to pass all the elements and therefore to make a copy of the array as a whole. How to solve this problem? Well, you pass the *name* of the array to the function. Doing this is equivalent to pass the pointer to the first element of the array, and that's really all you need. Once you know where the array starts you can move to the following elements using an external pointer. Remember that your function needs to know the dimension of the array, so pass this information to the function as well. 
